...
* The ```conn.commit()``` function is committing the database transaction. That means all the changes made to the database by the SQL commands executed by the curosr will be persisted in the database. Without calling this function, the changes to the database will not be permanent.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbOTAyMDU4MDg3LDE4NjA3MTYyNjUsNjk4MD

...
* The ```conn.commit()``` function is committing all the uncommitted changes made to the database by the SQL commands executed by the cursor. Without calling this function, the database changes made by the cursor object will not be permanent. Hence do not forget to call ```conn.commit``` after executing a database modification command like INSERT, DELETE, UPDATE.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTY4NzM0NDcwMywtODEzMzk4NDc4LDE4Nj

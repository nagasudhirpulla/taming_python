...
* The following is the explanation for each URL parameter
	...
	* `state` is an arbitrary string passed by client application to the OAuth server. OAuth server should provide this `state` to client application along with authorization code as a proof that the response sent request by the client application 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTU0NTU4NjgyMCwtMjA2NDA0ODk5NCwtND

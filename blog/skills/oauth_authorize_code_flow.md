...
* The following is the explanation for each URL parameter
	...
	* `state` is a string passed in the request by client application to the OAuth server. OAuth server should respond with this `state` along with authorization code as a proof that the response sent corresponds to that request only 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbNjgyNTM2OTIwLC0yMDY0MDQ4OTk0LC00Nj
